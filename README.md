# AAS-VOSK-BYDMate 1.2.8

AAS — офлайн-голосовой помощник для мультимедийных систем BYD/DiLink:

```text
кнопка руля → Vosk → двуязычный парсер → Android/BYD-действие → голосовой ответ
```

Приложение не использует облачное распознавание и не ведёт постоянную запись. После нажатия назначенной кнопки оно слушает одну команду, выполняет её и завершает сеанс.
...
- Android TextToSpeech с переключением `ru-RU` / `uk-UA` и выводом в BYD Voice stream 17;
- восстановление пакета `com.byd.autovoice.tts`, если его отключила старая версия helper;
- перебор доступных BYD/системных TTS-движков с повторной инициализацией и контролем запуска воспроизведения;
- кнопка проверки озвучивания в настройках;
...
5. Нажать «Проверить озвучивание».
