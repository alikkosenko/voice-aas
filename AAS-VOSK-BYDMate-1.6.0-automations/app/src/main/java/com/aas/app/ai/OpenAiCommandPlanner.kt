...
class OpenAiCommandPlanner(private val prefs: AppPrefs) {
    ...
    private fun mapCommand(item: JSONObject): VoiceCommand? {
        ...
        return when (action) {
            ...
            "mirror_heat_on" -> VoiceCommand.RearDefrostOn
            "mirror_heat_off" -> VoiceCommand.RearDefrostOff
            "front_defrost_on" -> VoiceCommand.FrontDefrostOn
            "front_defrost_off" -> VoiceCommand.FrontDefrostOff
            "climate_flow_only_on" -> VoiceCommand.ClimateFlowOnlyOn
            "climate_flow_only_off" -> VoiceCommand.ClimateFlowOnlyOff
            ...
            "seat_heating" -> VoiceCommand.SetSeatHeating(mapSeat(target), value.coerceIn(0, prefs.seatMaxLevel))
            "seat_ventilation" -> VoiceCommand.SetSeatVentilation(mapSeat(target), value.coerceIn(0, prefs.seatMaxLevel))
            ...
            "window_vent" -> VoiceCommand.SetWindowPosition(mapWindow(target), 10)
            "window_half" -> VoiceCommand.SetWindowPosition(mapWindow(target), 50)
            ...
            "play_youtube" -> text.takeIf { it.isNotEmpty() }?.let { VoiceCommand.PlayYoutube(it) }
            "search_music" -> text.takeIf { it.isNotEmpty() }?.let { VoiceCommand.SearchMusic(it) }
            "play_music" -> text.takeIf { it.isNotEmpty() }?.let { VoiceCommand.PlayMusic(it) }
    ...
    private fun systemPrompt(): String = SYSTEM_PROMPT
        .replace("{{SEAT_MAX}}", prefs.seatMaxLevel.toString())
        .replace("{{SEAT_MIDDLE}}", if (prefs.seatMaxLevel >= 5) "3" else "2")
    ...
    companion object {
        ...
        private val ALLOWED_ACTIONS = listOf(
            ...
            "rear_defrost_on", "rear_defrost_off", "mirror_heat_on", "mirror_heat_off",
            "front_defrost_on", "front_defrost_off", "climate_flow_only_on", "climate_flow_only_off",
            ...
            "window_open", "window_close", "window_position", "window_vent", "window_half",
            ...
            "navigate_to", "repeat_navigation", "open_navigator", "open_music", "search_music", "play_music",
...
7. Seat heat/vent levels are 0=off and 1..{{SEAT_MAX}}; middle={{SEAT_MIDDLE}}, maximum={{SEAT_MAX}}. Steering heat is 0..3. Fan is 1..7. Temperature is 16..30. Window position is 0..100.
...
- "обдуй лобовое": front_defrost_on. "выключи обдув лобового": front_defrost_off.
- "включи только вентиляцию без охлаждения": climate_flow_only_on.
- "проветри окна": window_vent target=all. "открой окна наполовину": window_half target=all.
- "включи обогрев зеркал": mirror_heat_on.
...
- "включи на ютубе Linkin Park Numb": play_youtube text="Linkin Park Numb". Use search_youtube for "найди/поищи" and play_youtube for "включи/поставь".
- "найди песню Кино Группа крови": search_music text="Кино Группа крови".
- "включи песню Кино Группа крови": play_music text="Кино Группа крови".
