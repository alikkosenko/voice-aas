 ...
 */
data class AutomationVehicleSnapshot(
...
class BydVehicleAdapter(
    ...
    /** Read-only values used by the event automation engine. */
    fun readAutomationSnapshot(): AutomationVehicleSnapshot {
        val connection = bootstrap.ensureRunning(enableAccessibility = false)
        if (!connection.connected) {
            return AutomationVehicleSnapshot(null, null, connection.message)
        }
        val socRead = readFloat(BydParameterCatalog.stateOfCharge)
        val speedRead = readFloat(BydParameterCatalog.vehicleSpeed)
        val soc = socRead.value.takeIf { socRead.error == null && it.isFinite() && it in 0f..100f }
            ?.toInt()?.coerceIn(0, 100)
        val speed = speedRead.value.takeIf { speedRead.error == null && it.isFinite() && it >= 0f }
        val errors = listOfNotNull(socRead.error?.let { "soc=$it" }, speedRead.error?.let { "speed=$it" })
        return AutomationVehicleSnapshot(soc, speed, errors.joinToString("; ").ifBlank { null })
    }

    fun checkStationaryForAutomation(): Pair<Boolean?, String> {
        val connection = bootstrap.ensureRunning(enableAccessibility = false)
        if (!connection.connected) return null to connection.message
        val speed = readFloat(BydParameterCatalog.vehicleSpeed)
        if (speed.error != null || !speed.value.isFinite()) {
            return null to (speed.error ?: "invalid speed raw=${speed.raw} decoded=${speed.value}")
        }
        return (speed.value <= 0.5f) to "vehicleSpeed=${format(speed.value)}"
    }
