# AAS-VOSK-BYDMate 1.6.0

AAS — офлайн-голосовой помощник для мультимедийных систем BYD/DiLink:

```text
события → AutomationEngine → локальные Android/BYD-действия
кнопка руля → Vosk → локальный парсер или OpenAI fallback → Android/BYD-действия
```
...
- голосовые ответы через выбранный системный TTS/RHVoice с отдельным выключателем;
...
### Событийные автоматизации

- отдельное Activity «Автоматизации» без голосовых триггеров;
- расписание, запуск AAS, интернет, питание, экран, аппаратная кнопка и пороги SOC;
- цепочки локальных команд, задержки, TTS, уведомления, приложения и URL;
- cooldown, stationary guard, run-once-per-boot, ручной тест, шаблоны и журнал;
- фоновые команды никогда не отправляются в OpenAI;
- при выключенном голосовом ассистенте Vosk не загружается.

Подробности: [`AUTOMATIONS.md`](AUTOMATIONS.md).
...
app/src/main/java/com/aas/app/commands/TextCommandExecutor.kt
app/src/main/java/com/aas/app/automation/AutomationsActivity.kt
app/src/main/java/com/aas/app/automation/AutomationEngine.kt
app/src/main/java/com/aas/app/automation/AutomationRepository.kt
...
## Event automations in 1.6.0

AAS includes an independent event-only automation screen. Rules can run from a
schedule, startup, connectivity, power/screen events, a hardware button or SOC
threshold crossings. They never use the microphone and never call OpenAI.
See `AUTOMATIONS.md`, `RELEASE_NOTES_1.6.0.md` and `VALIDATION_1.6.0.md`.
